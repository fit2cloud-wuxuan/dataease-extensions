package io.dataease.plugins.datasource.hiveV1.query;
 ...
 */
@Service("hiveV1QueryProvider")
public class HiveV1QueryProvider extends QueryProvider {
    @Resource
    private DatasetTableFieldMapper datasetTableFieldMapper;

    private final String UNIX_TIMESTAMP2 = "unix_timestamp(%s,'%s')";
    ...
    public String createSQLPreview(String sql, String orderBy) {
        return "SELECT tmp.* FROM (" + sqlFix(sql) + ") AS tmp " + " LIMIT 1000";
    }
    ...
    public String createQuerySQL(String table, List<DatasetTableField> fields, boolean isGroup, Datasource ds, List<ChartFieldCustomFilterDTO> fieldCustomFilter, List<DataSetRowPermissionsTreeDTO> rowPermissionsTree, List<DeSortField> sortFields) {
        ...
        if (CollectionUtils.isNotEmpty(fields)) {
            if (CollectionUtils.isNotEmpty(sortFields)) {
                List<DeSortField> differenceFields = sortFields.stream().filter(sortField -> fields.stream().noneMatch(field -> field.getOriginName().equals(sortField.getOriginName()))).collect(Collectors.toList());
                if (CollectionUtils.isNotEmpty(differenceFields)) {
                    fields.addAll(differenceFields);
                }
            }
            for (int i = 0; i < fields.size(); i++) {
                ...
                if (f.getDeExtractType() == DeTypeConstants.DE_TIME) {
                    ...
                    if (f.getDeType() == DeTypeConstants.DE_INT) {
                        ...
                        fieldName = String.format(HiveConstants.FROM_UNIXTIME, String.format(UNIX_TIMESTAMP2, originField, StringUtils.isNotEmpty(f.getDateFormat()) ? f.getDateFormat() : HiveConstants.DEFAULT_DATE_FORMAT), StringUtils.isNotEmpty(f.getDateFormat()) ? f.getDateFormat() : HiveConstants.DEFAULT_DATE_FORMAT);
                    } else {
                        fieldName = originField;
                    }
                } else {
                ...
                xFields.add(SQLObj.builder()
                        ...
                        .build());
                Optional.ofNullable(sortFields).orElse(new ArrayList<>()).forEach(sortField -> {
                    if (f.getOriginName().equals(sortField.getOriginName())) {
                        sortField.setOriginName(fieldAlias);
                    }
                });
            }
        }
    ...
    public String transTreeItem(SQLObj tableObj, DatasetRowPermissionsTreeItem item) {
        ...
        if (field.getDeType() == DeTypeConstants.DE_TIME) {
            if (field.getDeExtractType() == DeTypeConstants.DE_STRING || field.getDeExtractType() == 5) {
                whereName = String.format(HiveConstants.FROM_UNIXTIME, String.format(UNIX_TIMESTAMP2, originName, StringUtils.isNotEmpty(field.getDateFormat()) ? field.getDateFormat() : HiveConstants.DEFAULT_DATE_FORMAT), StringUtils.isNotEmpty(field.getDateFormat()) ? field.getDateFormat() : HiveConstants.DEFAULT_DATE_FORMAT);
            }
    ...
    public String getSQLWithPage(boolean isTable, String table, List<ChartViewFieldDTO> xAxis, List<ChartFieldCustomFilterDTO> fieldCustomFilter, List<DataSetRowPermissionsTreeDTO> rowPermissionsTree, List<ChartExtFilterRequest> extFilterRequestList, Datasource ds, ChartViewWithBLOBs view, PageInfo pageInfo) {
        String limit = ((pageInfo.getGoPage() != null && pageInfo.getPageSize() != null) ? " LIMIT " + pageInfo.getPageSize() : "");
        if (isTable) {
    ...
    public String transExtFilterList(SQLObj tableObj, List<ChartExtFilterRequest> requestList) {
        ...
        for (ChartExtFilterRequest request : requestList) {
            ...
            for (DatasetTableField field : fieldList) {
                ...
                if (field.getDeType() == DeTypeConstants.DE_TIME) {
                    if (field.getDeExtractType() == DeTypeConstants.DE_STRING || field.getDeExtractType() == 5) {
                        whereName = String.format(HiveConstants.FROM_UNIXTIME, String.format(UNIX_TIMESTAMP2, originName, StringUtils.isNotEmpty(field.getDateFormat()) ? field.getDateFormat() : HiveConstants.DEFAULT_DATE_FORMAT), format);
                    }
                    if (field.getDeExtractType() == DeTypeConstants.DE_INT || field.getDeExtractType() == 3 || field.getDeExtractType() == 4) {
                        String cast = String.format(HiveConstants.CAST, originName, HiveConstants.DEFAULT_INT_FORMAT) + "/1000";
    ...
    private SQLObj getXFields(ChartViewFieldDTO x, String originField, String fieldAlias) {
        ...
        if (x.getDeExtractType() == DeTypeConstants.DE_TIME) {
            if (x.getDeType() == 2 || x.getDeType() == 3) {
                ...
                fieldName = String.format(HiveConstants.FROM_UNIXTIME, String.format(UNIX_TIMESTAMP2, originField, format), format);
            } else {
                fieldName = originField;
            }
        } else {
            if (x.getDeType() == DeTypeConstants.DE_TIME) {
                ...
                if (x.getDeExtractType() == DeTypeConstants.DE_STRING) {
                    fieldName = String.format(HiveConstants.FROM_UNIXTIME, String.format(UNIX_TIMESTAMP2, originField, StringUtils.isNotEmpty(x.getDateFormat()) ? x.getDateFormat() : HiveConstants.DEFAULT_DATE_FORMAT), format);
                } else {
                    String cast = String.format(HiveConstants.CAST, originField, HiveConstants.DEFAULT_INT_FORMAT) + "/1000";
                    fieldName = String.format(HiveConstants.FROM_UNIXTIME, cast, format);
                }
            } else {
